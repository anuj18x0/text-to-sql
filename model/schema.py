...
class FactOrders(Base):
    ...
    order_status = Column(String(32), nullable=False, default="unknown", index=True)
    ...
    review = relationship(
        "DimReviews",
        primaryjoin="FactOrders.order_id == DimReviews.order_id",
        foreign_keys="FactOrders.order_id",
...
class DimReviews(Base):
    ...
    # Note: Removed ForeignKey("fact_orders.order_id") because fact_orders.order_id is no longer unique.
    # The relationship will still work at the ORM level.
    ...
    order = relationship(
        "FactOrders",
        primaryjoin="DimReviews.order_id == FactOrders.order_id",
        foreign_keys="FactOrders.order_id",
