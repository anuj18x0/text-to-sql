...
def load_dim_geography(geo_df: pd.DataFrame, engine) -> None:
    ...
    Normalises city names (title casing) BEFORE deduplication to ensure
    casing variations don't trigger integrity errors.
    """
    logger.info("Loading dim_geography (raw %d rows)...", len(geo_df))
    
    # Pre-normalise city names so that "SAO PAULO" and "Sao Paulo" match
    geo_df = geo_df.copy()
    geo_df["geolocation_city"] = geo_df["geolocation_city"].astype(str).str.strip().str.title()
...
def load_dim_reviews(reviews_df: pd.DataFrame, orders_df: pd.DataFrame, engine) -> None:
    """Load dim_reviews from olist_order_reviews_dataset.csv."""
    logger.info("Loading dim_reviews (%d rows)...", len(reviews_df))
    # Deduplicate by order_id (keep last review if multiple exist for a single order)
    reviews_dedup = reviews_df.drop_duplicates(subset=["order_id"], keep="last")
    records = [
        {
    ...
    records = [r for r in records if r["order_id"] in valid_orders]
    ...
    with Session(engine) as session:
        for chunk_start in range(0, len(records), 2000):
            ...
            session.commit()
            logger.info("  dim_reviews: inserted rows %d–%d", chunk_start, chunk_start + len(chunk))
...
def seed() -> None:
    ...
    engine = get_engine()
    logger.info("Dropping existing schema to apply fix...")
    from sqlalchemy import text
    with engine.begin() as conn:
        for table in ["query_log", "dim_reviews", "fact_orders", "dim_geography", "dim_sellers", "dim_products", "dim_users"]:
    ...
    load_dim_reviews(reviews_df, orders_df, engine)
